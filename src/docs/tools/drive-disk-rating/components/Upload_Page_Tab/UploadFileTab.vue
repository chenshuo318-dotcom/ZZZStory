<template>
  <div class="p-5 mb-5">
    <div
      class="border-2 border-dashed border-(--vp-c-divider) rounded-lg p-10 text-center cursor-pointer transition-all duration-300 bg-(--vp-c-bg) hover:border-(--main-color-1) hover:bg-(--vp-c-bg-soft)"
      :class="{
        'border-(--main-color-1) bg-(--vp-c-bg-soft)': isDragover,
      }"
    ...
    >
      ...
      <p class="text-lg font-medium mb-2 text-(--vp-c-text-1)">
        点击选择文件或拖拽文件到此处
      </p>
      <p class="text-sm text-(--vp-c-text-2)">支持 JSON 格式文件</p>
    </div>
    ...
    <div
      v-if="fileInfo"
      class="bg-(--vp-c-success-soft) border border-(--vp-c-success-1) rounded-md p-4 mt-4"
    >
      <div class="flex items-center gap-3">
        <div
          class="w-10 h-10 bg-(--vp-c-success-1) rounded-md flex items-center justify-center text-white font-bold"
        >
          {{ JSON }}
        </div>
        <div>
          <div class="font-semibold text-(--vp-c-text-1) mb-1">
            {{ fileInfo.name }}
          </div>
    ...
    <div
      v-if="error"
      class="bg-(--vp-c-danger-soft) border border-(--vp-c-danger-1) rounded-md p-4 mt-4 text-(--vp-c-danger-1)"
    >
      <strong class="font-semibold">错误：</strong>{{ error }}
    </div>

    <div v-if="isLoading" class="text-center py-10">
      <div
        class="w-10 h-10 mx-auto mb-4 border-4 border-(--vp-c-divider) border-t-4 rounded-full animate-spin"
      ></div>
      <p class="text-(--vp-c-text-1)">正在计算评分...</p>
    </div>

    <div class="mt-5">
      <p class="text-(--vp-c-text-2) mb-5">
